...
def fetch_all_withdrawals(token, cookies):
    ...
    query = """
    query WithdrawalList($limit: Int, $offset: Int) {
      user {
        withdrawalList(limit: $limit, offset: $offset) {
          id
          amount
          currency
          createdAt
    ...
    print(" [SYSTEM] Fetching full withdrawal history for accounting...")
    
    while True:
        try:
            resp = requests.post(url, json={"query": query, "variables": {"limit": limit, "offset": offset}}, headers=headers, cookies=cookie_dict)
            data = resp.json()
            withdrawals = data.get("data", {}).get("user", {}).get("withdrawalList", [])
    ...
    # Save to CSV
    filename = "company_withdrawals_report.csv"
    with open(filename, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["ID", "Amount", "Currency", "Date (GMT)", "Status"])
        for d in all_withdrawals:
            writer.writerow([d['id'], d['amount'], d['currency'], d['createdAt'], d['status']])
    
    print(f" [SUCCESS] Exported {len(all_withdrawals)} withdrawal records to {filename}")
...
if __name__ == "__main__":
    ...
    fetch_all_withdrawals(TOKEN, COOKIES)
