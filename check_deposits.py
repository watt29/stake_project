...
def fetch_deposits(token, cookies):
    ...
    # Common Stake query for deposit history
    query = """
    ...
    try:
        resp = requests.post(url, json={"query": query, "variables": variables}, headers=headers, cookies=cookie_dict)
        data = resp.json()
        if "data" in data and data["data"]["user"]["depositList"]:
            deposits = data["data"]["user"]["depositList"]
            for d in deposits:
                print(f"ID: {d['id']} | Amount: {d['amount']} {d['currency']} | Date: {d['createdAt']} | Status: {d['status']}")
        else:
            print("No deposits found or API error.")
